...
Spoof = true -- Spoofs Model and Name for video recording 


--Menu Settings
CustomMenuPosition = false 
NewX, NewYP = 0 , 0 --Place your copied menu poisiton here

CustomMenuSize = false 
NewXS, NewYS = 0 , 0 --Place your copied menu size here
...
CustomPos = UDim2.new(NewXP, 0, NewYP, 0)
if CustomMenuPosition == true then
game:GetService("CoreGui").STKv8S.MainFrameSet.Position = CustomPos
end
CustomSize = UDim2.new(NewXS, 0, NewYS, 0)

if CustomMenuSize == true then
game:GetService("CoreGui").STKv8S.MainFrameSet.Size = CustomSize
end
...
function TopSpoof()
if game.Workspace:FindFirstChild("Top10") then
...
if Player.name == game.Players.LocalPlayer.name then wait(.1)
Morph(UserId)
end end)
...
if Spoof == true then
Morph(483598592)
 game.Players.LocalPlayer.DisplayName = "Kliro" 
  game.Players.LocalPlayer.Name = "Ghostmode65" 
  TopSpoof()
end
...
return 16 end
return oldindex(self, method)end)
...
--LoadStrung Scripts/ Neat Scripts
--loadstring(game:HttpGet('https://raw.githubusercontent.com/Ghostmode65/STK-Bo2/master/STK-Menus/v8/Scripts/Not%20Mine!/Chat%20Bypass.lua'))()
--loadstring(game:HttpGet('https://raw.githubusercontent.com/Ghostmode65/STK-Bo2/master/STK-Menus/v8/Scripts/Not%20Mine!/KeyStrokes.lua'))()
--loadstring(game:HttpGet('https://raw.githubusercontent.com/Ghostmode65/STK-Bo2/master/STK-Menus/v8/Scripts/Not%20Mine!/Translater.lua'))()
...
function RemoveRadioActive()
for i,v in pairs(workspace.AREA51.RadioactiveArea:GetChildren()) do 
if v.name == 'Entrance' then  v:Destroy() end end  end 

function RemoveDamgeScripts() 
for i,v in pairs(workspace.AREA51:GetDescendants()) do 
if v.name == 'Damage' 
or v.name == 'ToxicFlow' 
or v.name == 'Damager'  
or v.name == 'Spinner Manager'  
or v.name == 'Toxic' 
...
v.Transparency = 1
 ...
 function Xray(ButtonN,DataPageN)
 for i,v in pairs(game:GetService("Workspace").AREA51:GetDescendants()) do 
 if v:IsA'Texture' or v:IsA'Decial' or v:IsA'Part' then 
if ButtonN:GetAttribute(DataPageN) == true then
 v.Transparency = .9 else  v.Transparency = 0 
  end end end end
...
--CC Cheats
 ...
 local PlayerWalk = game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed
 local PlayerJump = game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower

function ChangeWalkS(ButtonN,DataPageN,val,OrginData)
 local newval = val + PlayerWalk
OrginData:SetAttribute(DataPageN, newval)

PlayerWalk = OrginData:GetAttribute(DataPageN)
end

function ChangeJumpP(ButtonN,DataPageN,val,OrginData)
 local newval = val + PlayerJump
 OrginData:SetAttribute(DataPageN, newval)
 PlayerJump = OrginData:GetAttribute(DataPageN)
end


local GayAssUser = game.Players.LocalPlayer.Name

	function KeepWalkSpeed(ButtonN,DataPageN,OrginData)
game:GetService("Workspace")["Characters to kill"].ChildAdded:Connect(function(child)
    if child.name == GayAssUser and ButtonN:GetAttribute(DataPageN) == true then 
game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = OrginData:GetAttribute(DataPageN)
	end end) end 

	function KeepJumpPower(ButtonN,DataPageN,OrginData)
game:GetService("Workspace")["Characters to kill"].ChildAdded:Connect(function(child)
    if child.name == GayAssUser and ButtonN:GetAttribute(DataPageN) == true then 
game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = OrginData:GetAttribute(DataPageN)
	end end) end 


function RayGunSilence()
	local players = game:GetService("Players")
	local plr = players.LocalPlayer
	local mouse = plr:GetMouse()
	local camera = game.Workspace.CurrentCamera
	local teamcheck = true
    local Killz = game.Workspace.Killers 
local function ClosestPlayerToMouse()
    local target = nil
    local dist = math.huge
for i,v in pairs(Killz:GetChildren()) do
    if v.Name ~= plr.Name then
        if v.Parent.name == 'Killers' and v:FindFirstChild("Zombie") and v.Zombie.Health ~= 0 and v.name ~= 'Jane'
     then
        print('Found', v.name)
            local screenpoint = camera:WorldToScreenPoint(v.Torso.Position)
...
return target 
	...
	mt.__namecall = function(self,...)
		...
		if tostring(self) == "Missile Touch" and method == "FireServer" then
			args[1] = ClosestPlayerToMouse().Head
			args[2] = ClosestPlayerToMouse().Head.Position
           -- args[3] = 'RayGun'
			return self.FireServer(self, unpack(args))
		end
		return namecall(self,...)end end 

--Other Scripts Loadstring
...
Page5.Text = "Closet Cheats"
Page6.Text = "Other Cool Scripts"
...
function LoadPage1()
     togglePage("Page1",
 "X","Teleport Tool","Delete Tool",
 "No Parkour","Prop All Signs","Smart Delay",
 "Auto Comeplete","NCS_B8","NCS_B9",
...
function LoadPage2()
 ...
 "Toggle Fog","Toggle Doors","X-Ray","NHS_4",
 "NHS_5","NHS_6","NHS_7","NHS_8",
  "DataPage2",Page2)
end
...
function LoadPage4()
 ...
 "Defult Speed","1.5x Speed","2x Speed",
 "Defult Jump","1.5x Jump","2x Jump",
 "NCS_B10","NCS_B11","NCS_B12",
 "NCS_B13","NCS_B14","NCS_B15",
 "NCS_B16","NCS_B17","NCS_B18",
 "NCS_B19","NCS_B20","NCS_B21",
 ...
 "Player Esp","NHS_6","Save WalkSpeed","Save JumpPower",
  "DataPage4",Page4)
end

function LoadPage5()
 togglePage("Page5",
 "Defult Speed","+ 0.5 Speed","+1 Speed",
"Defult Jump","+ 0.5 Jump","+1 Jump",
 "X","???","NCS_B9",
 "NCS_B10","NCS_B11","NCS_B12",
 "NCS_B13","NCS_B14","NCS_B15",
 "NCS_B16","NCS_B17","NCS_B18",
...
function LoadPage6()
 togglePage("Page6",
 "Chat Translator","Chat Bypass","KeyStrokes",
 "NCS_B4","NCS_B5","NCS_B6",
 "NCS_B7","NCS_B8","NCS_B9",
 "NCS_B10","NCS_B11","NCS_B12",
 "NCS_B13","NCS_B14","NCS_B15",
...
function LoadPage8()
 togglePage("Page8",
 "Rejoin World","Dark Dex 4","Delete Gui",
 "NCS_B4","NCS_B5","NCS_B6",
 "NCS_B7","NCS_B8","NCS_B9",
 "NCS_B10","NCS_B11","NCS_B12",
 "NCS_B13","NCS_B14","NCS_B15",
...
function TouchStart()
      firetouchinterest(game.Players.LocalPlayer.Character.Torso,workspace.AREA51.Killhouse.StartTrigger, 0)
task.wait(.01)
 firetouchinterest(game.Players.LocalPlayer.Character.Torso,workspace.AREA51.Killhouse.StartTrigger, 1)
end
...
box = Instance.new('Part',workspace)
	box.Anchored = true
	box.CanCollide = true
	box.Size = Vector3.new(50,1,30)
	box.Position = Vector3.new(-258.9678192138672, 290.0048828125, 274.6300354003906)
	-- box.Touched:connect(function() end)
end

function DelayStart()
workspace.AREA51.Killhouse.StartTrigger.CFrame = CFrame.new(0,0,0)
	...
	box.Size = Vector3.new(.01,10,7)
	box.Position = Vector3.new(-205.1508331298828, 293.5002136230469, 250.6595001220703)
    box.Transparency = .99
...
function PropAll()
    for _,v in pairs(workspace.AREA51.Killhouse.Targets:GetDescendants()) do 
	if v.Name == 'TouchInterest' then 
    firetouchinterest(game.Players.LocalPlayer.Character.Torso,v.Parent, 0) end end end

   
function AtuoC()
   function TouchEnd()
      firetouchinterest(game.Players.LocalPlayer.Character.Torso,workspace.AREA51.Killhouse.EndTrigger, 0)
task.wait(.01)
 firetouchinterest(game.Players.LocalPlayer.Character.Torso,workspace.AREA51.Killhouse.EndTrigger, 1)
end
...
function GiveGun(Gunname)
    for _,GunGiver in pairs(game:GetService("Workspace").Weapons:GetDescendants()) do 
		...
		then firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart,GunGiver, 0)
        task.wait(2)
         game:GetService("Players").LocalPlayer.Character.Humanoid:EquipTool(game:GetService("Players").LocalPlayer.Backpack['M16A2/M203']) print('M16A2 Equiped') end end end
...
function SetCam(x,y,z)
    game.Workspace.CurrentCamera.CoordinateFrame =  CFrame.new(x,y,z,-0.999321, -0.0362516, 0.0065613 ,  0, 0.1781, 0.984012  ,  -0.0368405, 0.983344, -0.177979 )
end
...
TpKill(-214.02210998535156, 293.5002136230469, 267.104736328125)
TpKill(-284.2854309082031, 293.5002136230469, 265.2138671875)
TpKill(-284.29998779296875, 293.5002136230469, 276.857421875)
TpKill(-313.89581298828125, 293.5002136230469, 270.1319274902344)
TpKill(-314.4223327636719, 293.5002136230469, 264.6175537109375)
TpKill(-335.4555969238281, 293.5002136230469, 260.26385498046875)
TpKill(-333.9000549316406, 293.5002136230469, 277.359130859375)
TpKill(-302.08026123046875, 293.5002136230469, 300.71881103515625)

TpKill(-306.8141174316406, 293.5002746582031, 324.68212890625)
TpKill(-315.2351379394531, 293.50018310546875, 323.6159973144531)
TpKill(-330.8541564941406, 293.5002136230469, 327.9888610839844)
TpKill(-325.24151611328125, 293.5002136230469, 336.21746826171875)
TpKill(-323.5535888671875, 293.50018310546875, 351.2457580566406)
TpKill(-314.51849365234375, 293.50018310546875, 354.11724853515625)
TpKill(-312.1600341796875, 298.5002136230469, 360.9366455078125)
TpKill(-286.1640625, 293.5002136230469, 351.4903564453125)
TpKill(-286.1640625, 293.5002136230469, 351.4903564453125)
TpKill(-276.01177978515625, 293.50762939453125, 323.8824462890625)

TpKill(-220.25035095214844, 313.500244140625, 340.5166931152344)
TpKill(-214.66941833496094, 313.5002136230469, 337.936767578125)
...
if ButtonNumber == "CS_B4" then  NoParkour() end
if ButtonNumber == "CS_B5" then  PropAll() end
if ButtonNumber == "CS_B6" then  DelayStart() end
if ButtonNumber == "CS_B7" then AtuoC() end
...
if ButtonNumber == "HS_3" then ToggleVareint(HS_3,DATAPAGENUMBER) Xray(HS_3,DATAPAGENUMBER) end
if ButtonNumber == "HS_4" then ToggleVareint(HS_4,DATAPAGENUMBER) end
if ButtonNumber == "HS_5" then ToggleVareint(HS_5,DATAPAGENUMBER) end
if ButtonNumber == "HS_6" then ToggleVareint(HS_6,DATAPAGENUMBER) end
if ButtonNumber == "HS_7" then ToggleVareint(HS_7,DATAPAGENUMBER) end
...
if ButtonNumber == "CS_B2" then end
if ButtonNumber == "CS_B3" then end
if ButtonNumber == "CS_B4" then game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = 16 
CS_B4:SetAttribute(DATAPAGENUMBER,16) end
if ButtonNumber == "CS_B5" then ChangeWalkS(CS_B5,DATAPAGENUMBER,5,CS_B4) game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = CS_B4:GetAttribute(DATAPAGENUMBER) end
if ButtonNumber == "CS_B6" then  ChangeWalkS(CS_B6,DATAPAGENUMBER,10,CS_B4)  game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = CS_B4:GetAttribute(DATAPAGENUMBER)end
if ButtonNumber == "CS_B7" then game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = 50 
CS_B7:SetAttribute(DATAPAGENUMBER,50) end
if ButtonNumber == "CS_B8" then ChangeJumpP(CS_B8,DATAPAGENUMBER,5,CS_B7) game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = CS_B7:GetAttribute(DATAPAGENUMBER) end 
if ButtonNumber == "CS_B9" then ChangeJumpP(CS_B9,DATAPAGENUMBER,10,CS_B7)game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = CS_B7:GetAttribute(DATAPAGENUMBER)end 
if ButtonNumber == "CS_B10" then end
if ButtonNumber == "CS_B11" then end
if ButtonNumber == "CS_B12" then end
if ButtonNumber == "CS_B13" then end
...
if ButtonNumber == "HS_5" then ToggleVareint(HS_5,DATAPAGENUMBER) EspPlayers(HS_5,DATAPAGENUMBER) end
if ButtonNumber == "HS_6" then ToggleVareint(HS_6,DATAPAGENUMBER) end
if ButtonNumber == "HS_7" then ToggleVareint(HS_7,DATAPAGENUMBER)  KeepWalkSpeed(HS_7,DATAPAGENUMBER,CS_B4) end
if ButtonNumber == "HS_8" then ToggleVareint(HS_8,DATAPAGENUMBER)  KeepJumpPower(HS_8,DATAPAGENUMBER,CS_B7) end
LoadColorState(DATAPAGENUMBER)
end
...
if ButtonNumber == "CS_B1" then game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = 16 
CS_B1:SetAttribute(DATAPAGENUMBER,16)  end
if ButtonNumber == "CS_B2" then  ChangeWalkS(CS_B5,DATAPAGENUMBER,.5,CS_B1) game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = CS_B1:GetAttribute(DATAPAGENUMBER) end
if ButtonNumber == "CS_B3" then  ChangeWalkS(CS_B5,DATAPAGENUMBER,1,CS_B1) game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = CS_B1:GetAttribute(DATAPAGENUMBER) end
if ButtonNumber == "CS_B4" then game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = 50 
CS_B4:SetAttribute(DATAPAGENUMBER,50) end
if ButtonNumber == "CS_B5" then ChangeJumpP(CS_B8,DATAPAGENUMBER,.5,CS_B4) game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = CS_B4:GetAttribute(DATAPAGENUMBER) end 
if ButtonNumber == "CS_B6" then ChangeJumpP(CS_B8,DATAPAGENUMBER,1,CS_B4) game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = CS_B4:GetAttribute(DATAPAGENUMBER) end 
if ButtonNumber == "CS_B8" then end
if ButtonNumber == "CS_B9" then end
if ButtonNumber == "CS_B10" then end
if ButtonNumber == "CS_B11" then end
...
if ButtonNumber == "CS_B1" then GetChat() end
if ButtonNumber == "CS_B2" then BypassFilter() end
if ButtonNumber == "CS_B3" then  KeyStrokes() end
if ButtonNumber == "CS_B4" then end
if ButtonNumber == "CS_B5" then end
if ButtonNumber == "CS_B6" then end
if ButtonNumber == "CS_B7" then end
...
if ButtonNumber == "HS_3" then ToggleVareint(HS_3,DATAPAGENUMBER) GunChams(HS_3,DATAPAGENUMBER) end
if ButtonNumber == "HS_4" then ToggleVareint(HS_4,DATAPAGENUMBER) end
if ButtonNumber == "HS_5" then ToggleVareint(HS_5,DATAPAGENUMBER) end
if ButtonNumber == "HS_6" then ToggleVareint(HS_6,DATAPAGENUMBER) end
if ButtonNumber == "HS_7" then ToggleVareint(HS_7,DATAPAGENUMBER) end
...
if ButtonNumber == "CS_B3" then STKv8S:Destroy() end
if ButtonNumber == "CS_B4" then end
if ButtonNumber == "CS_B5" then end
if ButtonNumber == "CS_B6" then end
if ButtonNumber == "CS_B7" then end
...
game:GetService("CoreGui").STKv8S.MainFrameSet.ChildAdded:Connect(function(v)
task.wait(3)
if v.Parent:FindFirstChild('LocalScriptz') and 
v.Parent:FindFirstChild('Teleports') and 
v.Parent:FindFirstChild('Guns') and
v.Parent:FindFirstChild('Settings')then 
